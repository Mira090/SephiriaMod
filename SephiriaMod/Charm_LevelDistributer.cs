...
namespace SephiriaMod
{
    public class Charm_LevelDistributer : Charm_VariableMaxLevel
    {
        ...
        public override Loc.KeywordValue[] BuildKeywords(UnitAvatar avatar, int level, int virtualLevelOffset, bool showAllLevel, bool ignoreAvatarStatus)
        {
            ...
            {
            new Loc.KeywordValue("LEVEL", "+" + value, Charm_Basic.GetPositiveColor(virtualLevelOffset))
            };
        }
        protected override void OnUpdate()
        {
            ...
            if (flag)
            {
                ...
                using (new GridInventory.Permission(base.Inventory))
                {
                    if(Level > 0)
                    {
                        base.Inventory.AddDungeonTempLevel(Pos1, -Level);
                    }
                    Pos1 = new(xIdx, yIdx - 1);
                    Level = CurrentLevelToIdx();
                    base.Inventory.AddDungeonTempLevel(Pos1, Level);
                }
                base.Inventory.UpdatePing(Pos1);
            }
            if (enableFlag)
            {
                enableFlag = false;
        ...
        protected override void OnDisabledEffect()
        {
            ...
            if (Level > 0)
            {
                ...
                if (base.Inventory.GetWritePermission())
                {
                    base.Inventory.AddDungeonTempLevel(Pos1, -Level);
                    Pos1 = new(xIdx, yIdx - 1);
                    Level = -1;
