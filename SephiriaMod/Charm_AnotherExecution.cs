    ...
    public class Charm_AnotherExecution : Charm_StatusInstance
    {
        ...
        private void OnAttackUnitBeforeOperation(UnitAvatar avatar, DamageInstance damage)
        {
            ...
            if(Random.Range(0f, 100f) < execute)
            {
                ...
                var phy = base.NetworkAvatar.GetCustomStat(ECustomStat.PhysicalDamage);
                var fire = base.NetworkAvatar.GetCustomStat(ECustomStat.FireDamage);
                var ice = base.NetworkAvatar.GetCustomStat(ECustomStat.IceDamage);
                var lightning = base.NetworkAvatar.GetCustomStat(ECustomStat.LightningDamage);
        ...
        private void OnAttackUnit(UnitAvatar avatar, DamageInstance damage)
        {
            ...
            {
                if (!base.NetworkAvatar.IsDead && IsEffectEnabled)
                {
