...
namespace SephiriaMod
{
    public class Charm_FireCooldownRecovery : Charm_StatusInstance
    {
        ...
        public override Loc.KeywordValue[] BuildKeywords(UnitAvatar avatar, int level, int virtualLevelOffset, bool showAllLevel, bool ignoreAvatarStatus)
        {
            ...
            {
            new Loc.KeywordValue("REQUIRE", value, Charm_Basic.GetPositiveColor(virtualLevelOffset)),
        ...
        private void OnAttackUnit(UnitAvatar unit, DamageInstance damage)
        {
            if(damage.elementalType == EDamageElementalType.Fire)
            {
                ...
                base.NetworkAvatar.SetEffectHUDValue(GetCharmHUDID(), $"{count}/{requireCount.SafeRandomAccess(CurrentLevelToIdx())}");
        ...
        public override void SaveItemOnServer(ISaveData saveData)
        {
            ...
            saveData.SetInt($"CharmSaveData_FireCooldownRecovery_{base.Item.InstanceID}_Stack", count);
        }

        public override void LoadItemOnServer(ISaveData saveData)
        {
            base.LoadItemOnServer(saveData);
            count = saveData.GetInt($"CharmSaveData_FireCooldownRecovery_{base.Item.InstanceID}_Stack", 0);
        }
