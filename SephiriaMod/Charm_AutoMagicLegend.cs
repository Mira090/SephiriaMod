...
namespace SephiriaMod
{
    public class Charm_AutoMagicLegend : Charm_Basic
    {
        ...
        {
            NewItemOwnInstance newItemOwnInstance = base.NetworkAvatar.Inventory.FindItem(new ItemPosition(base.Item.XIdx, (sbyte)(base.Item.YIdx + offset)));
            magicCharm = null;
            slot = null;
        ...
        public override CharmConnectionData[] GetConnectedCharmPositions()
        {
            ...
            if (newItemOwnInstance == null || !newItemOwnInstance.Charm || !newItemOwnInstance.Charm.IsEffectEnabled || !(newItemOwnInstance.Charm is Charm_Magic charm_Magic))
            {
                ...
                {
                new CharmConnectionData(magicCharmIconSprite, new Vector2Int(base.Item.XIdx, base.Item.YIdx + offset))
                };
            }
        ...
        public override Sprite GetSubIconImage(ItemPosition pos, bool isInstance, int idx)
        {
            ...
            NewItemOwnInstance newItemOwnInstance = base.NetworkAvatar.Inventory.FindItem(new ItemPosition(pos.x, (sbyte)(pos.y + offset)));
            if (newItemOwnInstance != null && (bool)newItemOwnInstance.Charm && newItemOwnInstance.Charm.IsEffectEnabled && newItemOwnInstance.Charm is Charm_Magic charm_Magic)
            {
        ...
        private void Start()
        {
            TopdownActor = base.NetworkAvatar.GetComponent<TopdownActorRenderingMetadata>();
        ...
        protected override void Update()
        {
            ...
            if (isCasting)
            {
                ...
                {
                    isCasting = false;
                    ActiveSkill skillObject = magicCharm.FireCasting(base.NetworkAvatar.transform.position, castingPosition, TopdownActor.CenterYPos, 1);
            ...
            if (base.NetworkAvatar == null || base.NetworkAvatar.IsDead || base.NetworkAvatar is not PlayerAvatar player || !base.NetworkAvatar.IsInBattle)
            {
                return;
            }
            ...
            if (slot != null && slot.HasAnyAmmo && !slot.CurrentUsing && magicCharm.CanCast(magicCharm.NetworkAvatar) == ECanUseSkillResult.Succeeded)
            {
                if (castIntervalTimer.Update(Time.deltaTime))
                {
                    ...
                    SpriteFx spriteFx = (string.IsNullOrEmpty(magicCharm.ContainedMagic.castingCircleOverride) ? SpriteFx.Pool.Spawn(ActiveSkillDatabase.FindCastingCircleByClass(magicCharm.ContainedMagic.GetMajorClass()), base.NetworkAvatar.transform.position + new Vector3(0f, 0.001f)) : SpriteFx.Pool.Spawn(magicCharm.ContainedMagic.castingCircleOverride, base.NetworkAvatar.transform.position + new Vector3(0f, 0.001f)));

                    Melon<Core>.Logger.Msg("casting..." + (spriteFx == null));
                    spriteFx.SetParent(base.NetworkAvatar.transform);
