...
namespace SephiriaMod
{
    public class Charm_TuningForksMkII : Charm_Basic
    {
        ...
        public void Start()
        {
            ...
            {
                var ob = UnityEngine.Object.Instantiate(SephiriaPrefabs.PallasBigBullet);
                UnityEngine.Object.Destroy(ob.GetComponent<NetworkIdentity>());
        ...
        private static GameObject OnInstantiate(GameObject original, Vector3 position, Quaternion rotation)
        {
            ...
            var ob = UnityEngine.Object.Instantiate(bulletClient, position, rotation);
        ...
        public override Loc.KeywordValue[] BuildKeywords(UnitAvatar avatar, int level, int virtualLevelOffset, bool showAllLevel, bool ignoreAvatarStatus)
        {
            ...
            {
            new Loc.KeywordValue("DAMAGEPERCENT", value, Charm_Basic.GetPositiveColor(virtualLevelOffset)),
        ...
        protected override void OnConnected(int instanceID)
        {
            ...
            skillController = base.NetworkAvatar.GetComponent<SkillController>();
        }
        ...
        public void OnAttack(int idx)
        {
            ...
            if (remain > 0 && !NetworkAvatar.IsDead && IsEffectEnabled)
            {
                ...
                var bulletDamage = (float)(base.NetworkAvatar.GetCustomStat(ECustomStat.FireDamage) + base.NetworkAvatar.GetCustomStat(ECustomStat.IceDamage) + base.NetworkAvatar.GetCustomStat(ECustomStat.LightningDamage)) / 3f;
        ...
        protected override void OnDisabledEffect()
        {
            ...
            remain = 0;
            base.NetworkAvatar.DestroyEffectHUD(GetCharmHUDID());
        }
        ...
        private void OnBeginCastMagicServerside()
        {
            if (!enhancedWeaponDamage && cooldownTimer.Check())
            {
                ...
                base.NetworkAvatar.CreateEffectHUD(valueHUD_ID, GetCharmHUDID());
                remain = 3;
            }
        ...
        {
            if (!base.NetworkAvatar.IsDead && IsEffectEnabled && enhancedWeaponDamage && damageInstance.fromType == EDamageFromType.DirectAttack && !target.IsDead)
            {
                enhancedWeaponDamage = false;
                base.NetworkAvatar.DestroyEffectHUD(GetCharmHUDID());
                float num = (float)(base.NetworkAvatar.GetCustomStat(ECustomStat.FireDamage) + base.NetworkAvatar.GetCustomStat(ECustomStat.IceDamage) + base.NetworkAvatar.GetCustomStat(ECustomStat.LightningDamage)) / 3f;
                float num2 = (float)damagePercentByLevel.SafeRandomAccess(CurrentLevelToIdx()) / 100f;
                num *= num2;
                num += num * ((float)base.NetworkAvatar.GetCustomStat(ECustomStat.MagicDamageBonus) * 0.01f);
                DamageInstance damage = DamageInstance.GetDamage(base.NetworkAvatar, damageId, target.transform.position, 4294967295L, num, EDamageType.Slice, EDamageFromType.None, Vector2.zero, 0, 0f);
