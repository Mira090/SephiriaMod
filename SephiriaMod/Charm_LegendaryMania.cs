    ...
    public class Charm_LegendaryMania : Charm_Basic
    {
        ...
        public override string GetEffectString(int idx, int level, int virtualLevelOffset, bool showAllLevel)
        {
            if (!showStatsEffectStringFirst)
            {
                ...
                Color color = ((statusInstance.Value < 0) ? Charm_Basic.GetNegativeColor(virtualLevelOffset) : Charm_Basic.GetPositiveColor(virtualLevelOffset));
        ...
        protected override void OnEnabledEffect()
        {
            ...
            if (!(base.NetworkAvatar.Inventory == null))
            {
                ...
                savedCount = base.NetworkAvatar.Inventory.charms.Values.Count(charm => charm.Item.Entity.rarity == EItemRarity.Legend);
            }
            if (instances != null)
            {
