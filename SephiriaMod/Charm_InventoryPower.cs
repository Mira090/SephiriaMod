    ...
    public class Charm_InventoryPower : Charm_VariableMaxLevelWhitePaper
    {
        ...
        public override string GetEffectString(int idx, int level, int virtualLevelOffset, bool showAllLevel)
        {
            if(indexes.Count == 0)
            {
                ...
                Color color2 = ((statusInstance2.Value < 0) ? Charm_Basic.GetNegativeColor(virtualLevelOffset) : Charm_Basic.GetPositiveColor(virtualLevelOffset));
        ...
        protected override void OnUpdate()
        {
            ...
            for (int x = -1; x <= 1; x++)
            {
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    NewItemOwnInstance newItemOwnInstance = base.NetworkAvatar.Inventory.FindItem(pos);
                    ...
                    {
                        base.Inventory.ForceRemoveItem(pos.x, pos.y);
                        Inventory.AddItemAtPosition(new ItemMetadata(instanceId, base.Item.EntityID, 1), pos);
                    }
                    return;
                }
            }
        ...
        public override void SaveItemOnServer(ISaveData saveData)
        {
            ...
            saveData.SetInt($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Stack", indexes.Count);
            for(int q= 0; q < indexes.Count; q++)
            {
                saveData.SetInt($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Stack" + q, indexes[q]);
            }
            if(category != null)
            {
                saveData.SetString($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Category", category);
            }
        }
        public override void LoadItemOnServer(ISaveData saveData)
        {
            base.LoadItemOnServer(saveData);
            var count = saveData.GetInt($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Stack", 0);
            indexes.Clear();
            for(int q = 0; q < count; q++)
            {
                indexes.Add(saveData.GetInt($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Stack" + q, -1));
                if (indexes[^1] == -1)
                {
                    indexes.RemoveAt(indexes.Count - 1);
                }
            }
            var cate = saveData.GetString($"CharmSaveData_InventoryPower_{base.Item.InstanceID}_Category", null);
