...
namespace SephiriaMod
{
    public class Charm_DrunkGuardian : Charm_StatusInstance
    {
        ...
        public override Loc.KeywordValue[] BuildKeywords(UnitAvatar avatar, int level, int virtualLevelOffset, bool showAllLevel, bool ignoreAvatarStatus)
        {
            ...
            new Loc.KeywordValue("MINUS", value3 + "%", Charm_Basic.GetNegativeColor(virtualLevelOffset))
            };
        }
        private void Awake()
        {
        ...
        private void OnAttackUnit(UnitAvatar avatar, DamageInstance damage)
        {
            if(damage.fromType == EDamageFromType.DirectAttack)
            {
                ...
                {
                    DamageInstance d = DamageInstance.GetDamage(base.NetworkAvatar, damageId, avatar.transform.position, 4294967295L, customStatUnsafe, EDamageType.Slice, EDamageFromType.None, Vector2.zero, 0, 0f);
